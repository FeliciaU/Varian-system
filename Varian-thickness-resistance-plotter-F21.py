...
filename_thickness='SmN-in-situ-130916-thickness-cleaned.txt'

file_location='/home/felicia/Desktop/GoogleDrive/PhD/SmN data/Varian/F21'
Sample_name='F21'
Comment='Thickness+resistance'
SEM_thickness=63.3  #SEM measured thickness in nm
...
Res_data=Res_data_2[Res_data_2["Time(s)"] < 610]    # Filters away data with fit values under Fit_value
...
Thick_data=Thick_data_raw['Thickness(A)']*SEM_thickness/100/0.530
Thick_data_time=Thick_data_raw[Thick_data_raw['Time(s)'] < 610]
print(Thick_data,Thick_data_time)

#Now interpolate the data so they can be plotted against each other. From time 0 to
new_time=np.linspace(85,600,1000)
...
plt.title('Resistance vs inverse thickness for F21')

data=np.vstack((Thick_data_interp,Res_data_interp)).T

with open('F21-Thickness-resistance-interpolated.txt', 'wb') as f:
